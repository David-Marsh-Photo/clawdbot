...
export async function runAgentTurnWithFallback(params: {
  ...
  const toolStartTimes = new Map<string, { start: number; name: string }>();
  ...
  while (true) {
    try {
            ...
            onAgentEvent: async (evt) => {
              ...
              if (evt.stream === "tool") {
                ...
                if (phase === "start") {
                  ...
                  toolStartTimes.set(toolCallId, { start: Date.now(), name: toolName });
                }
                ...
                if (phase === "update") {
                  ...
                  if (params.opts?.onToolStatusUpdate && toolCallId) {
                    ...
                    const startTime = toolStartTimes.get(toolCallId)?.start ?? now;
                    ...
                    if (elapsedMs >= 10_000 && now - lastUpdate >= TOOL_STATUS_THROTTLE_MS) {
                      ...
                      const p = params.opts.onToolStatusUpdate({
                        toolName,
                        toolCallId,
                        status: "running",
                        summary,
                ...
                if (phase === "end") {
                  ...
                  if (params.opts?.onToolStatusUpdate) {
                    const startTime = toolStartTimes.get(toolCallId)?.start;
                    const p = params.opts.onToolStatusUpdate({
                      toolName,
                      toolCallId,
                      status: "completed",
      ...
      // Cleanup any running tools to avoid stuck status messages
      if (params.opts?.onToolStatusUpdate) {
        for (const [toolCallId, info] of toolStartTimes) {
          const p = params.opts.onToolStatusUpdate({
            toolName: info.name,
            toolCallId,
            status: "error",
